...
// Contains shop name (left) + quick actions: sync, dark/light toggle, settings (right)
...
export function AppHeader({ title, showSync = true, showToggle = true, showSettings = true }: Props) {
  ...
  const loadPendingSync = useCallback(async () => {
    ...
    try {
      ...
      // DB not ready or other error — silently ignore, keep previous count
    } finally {
  ...
  useEffect(() => {
    loadPendingSync()
    const interval = setInterval(loadPendingSync, 30_000)
