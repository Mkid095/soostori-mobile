...
export function JoinShopSheet({ visible, onClose, onSuccess }: Props) {
  ...
  const { serverIp, setServerIp, deviceName, setDeviceName, step, setStep, errorMsg, loading, handleConnect, reset } = useLanDiscovery()
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={handleClose}>
      <View style={{ flex: 1, backgroundColor: bg }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: border }}>
          ...
          <TouchableOpacity onPress={handleClose}><Text style={{ color: brand, fontWeight: '700', fontSize: 15 }}>Cancel</Text></TouchableOpacity>
        </View>
        ...
        <View style={{ padding: 24, gap: 20 }}>
          {step === 'ip_entry' && (
            <>
              <View style={{ alignItems: 'center', marginBottom: 8 }}>
                ...
                <Text style={{ fontSize: 15, color: textSecondary, textAlign: 'center' }}>Enter the shop computer's IP address to connect</Text>
              </View>

              <View>
                <Text style={{ fontSize: 13, fontWeight: '600', color: textSecondary, marginBottom: 6, textTransform: 'uppercase' }}>Server IP Address</Text>
                <TextInput
                  style={{ backgroundColor: card, borderWidth: 1, borderColor: border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 16, color: text }}
          ...
          {step === 'waiting' && (
            <View style={{ alignItems: 'center', paddingTop: 40, gap: 16 }}>
              ...
              <Text style={{ fontSize: 14, color: textSecondary, textAlign: 'center' }}>Ask the shop owner or manager to approve this device in their POS system</Text>
            </View>
          )}
          ...
          {step === 'success' && (
            <View style={{ alignItems: 'center', paddingTop: 40, gap: 16 }}>
              ...
              <Text style={{ fontSize: 14, color: textSecondary, textAlign: 'center' }}>Device paired successfully</Text>
            </View>
          )}
          ...
          {step === 'error' && (
            <View style={{ alignItems: 'center', paddingTop: 40, gap: 16 }}>
              ...
              <TouchableOpacity style={{ backgroundColor: brand, paddingVertical: 14, borderRadius: 12, paddingHorizontal: 32 }} onPress={() => setStep('ip_entry')}>
